...
namespace {
extern "C" void ebTask(void *aParameters) { tb::TinyBus::Instance().RunMainLoop(); }
} // namespace
...
bool handleEvent(const Subscriber *apSubscriber, Event &arEvent)
{
    ...
    for (i = 0; i < apSubscriber->tableRowCount; i++)
    {
        ...
        if ((arEvent.event == row->event) &&
            ((apSubscriber->currentState == row->state) || (row->state == TB_STATE_ANY)) &&
            ((row->conditionCheck == nullptr || (row->conditionCheck)())))
        {
            // call exit action from last state (only, if the next state is new)
            if (apSubscriber->exitAction != nullptr && row->nextState != TB_STATE_KEEP)
            {
...
extern "C" tbError tb_subscribe(Subscriber *apSubscriber) { return TinyBus::Instance().Subscribe(apSubscriber); }

extern "C" tbError tb_publish(const Event *apEvent) { return TinyBus::Instance().Publish(apEvent); }
...
TinyBus::TinyBus()
{
    ...
    mSubscriberCount = 0;
    mpSubscriber     = (Subscriber **)tb_malloc(CONFIG_TINYBUS_MAX_SUBSCRIBERS * sizeof(Subscriber *));
    // create event queue
